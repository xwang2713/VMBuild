...
#apt-get upgrade -y
apt-get install -y linux-headers-3.13.0-24-generic build-essential dkms --fix-missing

apt-get install -y xserver-xorg xbitmaps xterm --fix-missing
apt-get -y -f install

apt-get install -y python2.y libpython2.7 python3.4 libpython3.4 --fix-missing
apt-get -y -f install
...
sleep 1
apt-get install -y libboost-regex1.54.0 libicu52 libxslt1.1 libxerces-c3.1 g++ expect libarchive13 openjdk-8-jdk r-base r-cran-rcpp libv8-dev --fix-missing
apt-get -y -f install

apt-get install -y git libapr1 libaprutil1 libtbb2 libatlas3-base  libmemcached10 libmemcachedutil2 mysqlclient18 --fix-missing
apt-get -y -f install
